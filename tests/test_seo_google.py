from pages.data_google import query_new, query_new_ekb
from pages.data_google import query_new_novosibirsk, query_new_kazan, query_new_samara
from pages.data_google_second import query_new_ufa, query_new_krasnodar, query_new_msk, query_new_spb
import time
...
def test_find_competitors_and_website_rank_kazan(driver):
    ...
    try:
        df.to_excel('google_kazan.xlsx', index=False)  # Пытаемся сохранить файл
        print("Результаты успешно сохранены в google_novosibirsk.xlsx.")
    except Exception as e:
        print(f"Не удалось сохранить результаты: {e}")
...
def test_find_competitors_and_website_rank_samara(driver):
    ...
    try:
        df.to_excel('google_samara.xlsx', index=False)  # Пытаемся сохранить файл
        print("Результаты успешно сохранены в google_novosibirsk.xlsx.")
    except Exception as e:
        print(f"Не удалось сохранить результаты: {e}")